...
def run_qa_evaluation(
    ...
    try:
        ...
        logging.info("=== EVALUATING QA DATASET ===")
        ...
        if missing_columns:
            logging.error(
                f"Missing required columns in QA dataframe: {missing_columns}"
            )
            return {}
    ...
    for comp in competitors:
        ...
        for campaign in campaigns:
            ...
            for index, row in campaign_df.iterrows():
                ...
                if not stacked_path.exists():
                    logging.warning(
                        f"    {cell_id}: Stacked image not found: {stacked_path}"
                    )
                    skipped_count += 1
                    continue
                ...
                if original_image_key.startswith("t"):
                    time_part = original_image_key[
                        1:
                    ]  # Remove 't' prefix (e.g., "002")
                    ...
                    gt_row = gt_df[
                        (
                            gt_df["composite_key"].str.contains(
                                original_image_key, na=False
                            )
                        )
                ...
                if gt_row.empty:
                    logging.warning(
                        f"    {cell_id}: No GT found for {original_image_key} in campaign {campaign}"
                    )
                    skipped_count += 1
                    continue
            ...
            logging.info(
                f"    Campaign '{campaign}': Processed {processed_count}, Skipped {skipped_count} images"
            )
            ...
            if campaign_scores:
                ...
                logging.info(
                    f"    Campaign '{campaign}' avg Jaccard: {campaign_avg:.4f} (from {len(campaign_scores)} scores)"
                )
            else:
        ...
        if competitor_all_scores:
            ...
            logging.info(
                f"  Overall avg Jaccard for '{comp}': {overall_avg:.4f} (from {len(competitor_all_scores)} scores)"
            )
        else:
    ...
    valid_competitors = sum(
        1
        for comp in competitors
        if not pd.isna(overall_averages.get(comp, float("nan")))
    )
    logging.info(
        f"Competitors with valid scores: {valid_competitors}/{len(competitors)}"
    )
    ...
    if output:
        save_qa_results_to_csv(
            all_results, per_campaign_averages, overall_averages, qa_df, output
        )
...
def save_qa_results_to_csv(
    all_results, per_campaign_averages, overall_averages, qa_df, output_path
):
...
def integrate_results(qa_dataframe_path, qa_results_list):
    ...
    for results_path in qa_results_list:
        ...
        # splits must match
        assert((df_qa[p_utils.SPLITS_COLUMN].values == df_res[p_utils.SPLITS_COLUMN].values).all())
        col_prefix = "QA-"
