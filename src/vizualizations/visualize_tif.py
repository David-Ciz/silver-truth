...
def visualize_multiple_labels(file_path, save_png=True):
    ...
    axes[0].set_title(
        f"Segmentation with {len(unique_labels)-1} objects\n(Remapped for visualization)"
    )
    axes[0].axis("off")
    ...
    contour_colors = plt.cm.Set3(
        np.linspace(0, 1, len(unique_labels) - 1)
    )  # Exclude background
    ...
    for i, label in enumerate(unique_labels[1:]):  # Skip background (0)
        if np.any(mask == label):
            _ = axes[1].contour(
                mask == label, levels=[0.5], colors=[contour_colors[i]], linewidths=2
            )
