...
CELL_ID_COLUMNS = [
    "gt_image",
    "label",
]  # columns that uniquely indentify a cell from a QA parquet
...
def add_split_type(parquet_path: str, build_opt: dict) -> str:
    ...
    groups_count = (
        df.shape[0] if is_ensemple else df[CELL_ID_COLUMNS].drop_duplicates().shape[0]
    )
    ...
    output_parquet_path = (
        parquet_path[:filetype_pos] + parquet_suffix + parquet_path[filetype_pos:]
    )
...
def _get_uniques(parquet_path) -> np.ndarray:
    ...
    df = ddc.load_dataframe_from_parquet_with_metadata(parquet_path)
    uniques_col = ",".join(CELL_ID_COLUMNS)
    df[uniques_col] = df[CELL_ID_COLUMNS[0]] + df[CELL_ID_COLUMNS[1]].apply(
        lambda x: "%04d" % (x)
    )
    uniques = (
        df[[uniques_col, SPLITS_COLUMN]].drop_duplicates().sort_values(by=uniques_col)
    )
