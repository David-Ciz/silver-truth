...
def demo_detailed_evaluation():
    ...
    if Path(demo_file).exists():
        ...
        comp_stats = df.groupby('competitor')['jaccard_score'].agg(['count', 'mean', 'std']).round(4)
        print(comp_stats)
        ...
        if 'cell_area_gt' in df.columns:
            ...
            print(f"   Correlation between cell size and Jaccard score: {correlation:.4f}")
            
            # Size bins
            df['size_bin'] = pd.cut(df['cell_area_gt'], bins=[0, 100, 500, 1000, float('inf')], 
        ...
        print(f"     python detailed_evaluation.py dataframes/BF-C2DL-MuSC_dataset_dataframe.parquet --output {demo_file}")
...
def demo_analysis_workflow():
    ...
    print("   python detailed_evaluation.py dataframes/BF-C2DL-MuSC_dataset_dataframe.parquet")
    
    print("\n2. Analyze results:")
    print("   python analyze_detailed_results.py detailed_BF-C2DL-MuSC_all_competitors.parquet")
    
    print("\n3. For batch processing:")
    print("   python run_all_detailed_evaluations.py")
    
    print("\n4. Integration with existing workflow:")
    print("   python evaluation.py evaluate-competitor dataframes/BF-C2DL-MuSC_dataset_dataframe.parquet --detailed")
...
def create_sample_visualization():
    ...
    for comp in competitors:
        ...
        axes[1,0].scatter(comp_data['cell_area_gt'], comp_data['jaccard_score'], 
    ...
    axes[1,1].axis('off')
    summary_stats = df_demo.groupby('competitor')['jaccard_score'].agg(['mean', 'median', 'std']).round(3)
    table_data = []
    for comp in summary_stats.index:
        table_data.append([comp, summary_stats.loc[comp, 'mean'], 
                          summary_stats.loc[comp, 'median'], summary_stats.loc[comp, 'std']])
...
def main():
    ...
    sample_df = create_sample_visualization()
