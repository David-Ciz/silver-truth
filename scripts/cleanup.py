...
def find_files_to_delete():
    ...
    files_to_delete = {"dataframes": [], "jobfiles": [], "fusion": []}
    ...
    for category, files in files_to_delete.items():
        if files:
            ...
            for i, file_path in enumerate(files[:3]):
                ...
                print(
                    f"   - {os.path.basename(file_path)} ({format_file_size(file_size)})"
                )
...
def delete_files(file_list, category_name, dry_run=False, confirm=False):
        ...
        if os.path.exists(file_path):
            ...
            if dry_run:
                ...
                try:
                    os.remove(file_path)
                    print(
                        f"Deleted: {os.path.basename(file_path)} ({format_file_size(file_size)})"
                    )
                    deleted_count += 1
                except OSError as e:
    ...
    if not dry_run and deleted_count > 0:
        print(
            f"Successfully deleted {deleted_count} {category_name} files ({format_file_size(total_size)})"
        )
...
Examples:
  ...
  python cleanup.py --confirm          # Ask for confirmation
    ...
    parser.add_argument(
        "--dataframes", action="store_true", help="Only remove dataframe files"
    )
    parser.add_argument("--jobfiles", action="store_true", help="Only remove job files")
    parser.add_argument(
        "--fusion", action="store_true", help="Only remove fusion results"
    )
    parser.add_argument(
        "--dry-run",
        action="store_true",
