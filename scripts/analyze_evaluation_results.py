...
def create_summary_statistics(df):
    ...
    dataset_stats = (
        df.groupby("dataset")["jaccard_score"]
...
def create_visualizations(df, output_dir):
    ...
    try:
        ...
        logging.warning(
            "Skipping visualizations. Install matplotlib and seaborn to enable visualizations."
        )
        return
    ...
    df_sorted = df.copy()
    dataset_means = (
        df.groupby("dataset")["jaccard_score"].mean().sort_values(ascending=False)
    )
    df_sorted["dataset"] = pd.Categorical(
        df_sorted["dataset"], categories=dataset_means.index, ordered=True
    )
    ...
    if df["dataset"].nunique() <= 20 and df["competitor"].nunique() <= 30:
        ...
        sns.heatmap(
            ...
            annot=False,
            cmap="viridis",
            cbar_kws={"label": "Mean Jaccard Score"},
        )
        plt.xlabel("Competitor")
        plt.ylabel("Dataset")
        ...
        plt.tight_layout()
        plt.savefig(
            viz_dir / "dataset_competitor_heatmap.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
...
def save_summary_report(summary_stats, df, output_dir):
    ...
    with open(report_path, "w") as f:
        ...
        f.write("-" * 40 + "\n")
        top_datasets = (
            summary_stats["per_dataset"].sort_values("mean", ascending=False).head(10)
        )
...
def main():
    ...
    if df is None:
        logging.error(
            "No evaluation results found. Make sure to run 'python run_all_evaluations.py' first."
        )
        return
    ...
    logging.info(f"{'='*50}")
    logging.info(
        f"Total evaluations processed: {summary_stats['overall']['total_evaluations']}"
    )
    logging.info(f"Datasets analyzed: {summary_stats['overall']['unique_datasets']}")
    logging.info(
        f"Competitors analyzed: {summary_stats['overall']['unique_competitors']}"
    )
    logging.info(
        f"Overall mean Jaccard score: {summary_stats['overall']['mean_jaccard']:.4f}"
    )
    logging.info(
        f"Overall median Jaccard score: {summary_stats['overall']['median_jaccard']:.4f}"
    )

    best_competitor = (
        summary_stats["per_competitor"].sort_values("mean", ascending=False).index[0]
    )
    best_score = (
        summary_stats["per_competitor"]
    ...
    logging.info(
        f"Best performing competitor: {best_competitor} (mean: {best_score:.4f})"
    )

    best_dataset = (
        summary_stats["per_dataset"].sort_values("mean", ascending=False).index[0]
    )
    best_dataset_score = (
        summary_stats["per_dataset"]
        .sort_values("mean", ascending=False)["mean"]
        .iloc[0]
    )
    logging.info(
        f"Best performing dataset: {best_dataset} (mean: {best_dataset_score:.4f})"
    )
