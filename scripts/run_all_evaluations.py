...
def main():
    ...
    if not parquet_files:
        logging.warning(
            f"No dataframe files found matching pattern: {DATAFRAMES_PATTERN}"
        )
    ...
    for parquet_file in sorted(parquet_files):
        ...
        if result:
            ...
            _ = extract_dataset_name(parquet_file)
            try:
                import pandas as pd
                ...
                if "gt_image" not in df.columns:
                    ...
                    logging.warning(
                        f"No GT available for: {parquet_file} (tracking-only dataset)"
                    )
                else:
                    failed += 1
    ...
    if failed > 0:
        ...
        if successful > 0:
            ...
            logging.warning(
                "No evaluations could be performed (no datasets with segmentation GT)"
            )
            sys.exit(1)
