...
mask2_crop = (img2_crop > 0).astype(
    np.uint8
) * 255  # Assuming any non-zero value is the mask
...
jaccard_crop = (
    np.sum(intersection_crop) / np.sum(union_crop) if np.sum(union_crop) > 0 else 0.0
)
...
jaccard_full = (
    np.sum(intersection_full) / np.sum(union_full) if np.sum(union_full) > 0 else 0.0
)
