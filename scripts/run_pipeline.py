import click
import logging
from silver_truth.cli import preprocessing
...
@click.option(
    ...
    help="Path to raw dataset folder (e.g. inputs-2020-07)",
)
@click.option(
    "--output-dir",
    required=True,
    type=click.Path(),
    help="Where to save synchronized data",
)
...
@click.option(
    ...
    required=True,
    type=click.Path(exists=True),
    help="Path to the generated job file",
)
...
def run_all():
    ...
    logging.warning(
        "This is an example pipeline. Please configure it in scripts/run_pipeline.py"
    )
    pass
