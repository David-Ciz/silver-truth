...
def main():
    ...
    try:
        cmd = [sys.executable, "convert_qa_to_parquet.py", "batch-convert"]
        ...
        if detailed_qa_dir.exists():
            ...
            if parquet_files:
                logging.info(
                    f"📊 Running analysis on {len(parquet_files)} parquet files"
                )

                for parquet_file in parquet_files:
                    try:
                        dataset_name = parquet_file.stem.replace(
                            "_detailed_qa_cells", ""
                        )
                        ...
                        if result.returncode == 0:
                            ...
                            logging.warning(
                                f"⚠️ Analysis failed for {dataset_name}: {result.stderr}"
                            )
