...
class DetailedResultsAnalyzer:
    ...
    def _validate_data(self):
        ...
        logging.info(
            f"Data validation passed. Dataset contains {len(self.df)} cell evaluations"
        )
    ...
    def competitor_comparison(self) -> pd.DataFrame:
        ...
        """
        competitor_stats = (
            self.df.groupby("competitor")["jaccard_score"]
        ...
        zero_scores = (
            self.df[self.df["jaccard_score"] == 0].groupby("competitor").size()
        )
        ...
        high_scores = (
            self.df[self.df["jaccard_score"] > 0.7].groupby("competitor").size()
        )
    ...
    def cell_size_analysis(self) -> Dict:
        ...
        size_analysis = (
            self.df.groupby("size_category")["jaccard_score"]
    ...
    def print_summary_report(self):
        ...
        print(
            f"   Zero scores: {stats['zero_scores_count']:,} ({stats['zero_scores_percentage']:.1f}%)"
        )
        print(
            f"   High scores (>0.7): {stats['high_scores_count']:,} ({stats['high_scores_percentage']:.1f}%)"
        )
    ...
    def create_visualizations(self, output_dir: str = "detailed_analysis"):
        ...
        ax1.axvline(
            self.df["jaccard_score"].mean(),
            color="red",
            linestyle="--",
            label=f'Mean: {self.df["jaccard_score"].mean():.3f}',
        )
        ax1.axvline(
            self.df["jaccard_score"].median(),
            color="orange",
            linestyle="--",
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / "jaccard_distribution.png", dpi=300, bbox_inches="tight"
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / "competitor_comparison_boxplot.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / "competitor_comparison_violin.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        for i, comp in enumerate(competitors):
            ...
            plt.scatter(
                comp_data["cell_area_gt"],
                comp_data["jaccard_score"],
                alpha=0.6,
                label=comp,
                color=colors[i],
                s=20,
        ...
        plt.tight_layout()
        plt.savefig(
            output_path / "size_vs_jaccard_by_competitor.png",
            dpi=300,
            bbox_inches="tight",
        )
        plt.close()
        ...
        if "dataset_name" in self.df.columns and self.df["dataset_name"].nunique() > 1:
            pivot_data = (
                self.df.groupby(["competitor", "dataset_name"])["jaccard_score"]
                .mean()
                .unstack()
            )
            ...
            plt.tight_layout()
            plt.savefig(
                output_path / "performance_heatmap.png", dpi=300, bbox_inches="tight"
            )
            plt.close()
        ...
        if "size_category" in self.df.columns:
            plt.figure(figsize=(12, 8))
            size_comp = (
                self.df.groupby(["competitor", "size_category"])["jaccard_score"]
                .mean()
                .unstack()
            )
            ...
            plt.xticks(rotation=45)
            plt.legend(
                title="Cell Size Category", bbox_to_anchor=(1.05, 1), loc="upper left"
            )
            plt.tight_layout()
            plt.savefig(
                output_path / "performance_by_size_category.png",
                dpi=300,
                bbox_inches="tight",
            )
            plt.close()
...
@click.option(
    "--output-dir", default="detailed_analysis", help="Directory for analysis outputs"
)
@click.option(
    "--save-summary", is_flag=True, help="Save summary statistics to CSV file"
)
def analyze_detailed_results(
    detailed_results_path: str, output_dir: str, save_summary: bool = False
):
    """
    ...
    try:
        ...
        _ = analyzer.create_visualizations(output_dir)
