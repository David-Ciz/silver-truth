...
def visualize_with_distinct_colors(file_path, save_png=True):
    ...
    cbar.set_ticklabels(["BG" if label == 0 else f"{label}" for label in unique_labels])
...
if __name__ == "__main__":
    if len(sys.argv) != 2:
        ...
        print(
            "Creates a comprehensive analysis of segmentation masks with distinct colors."
        )
        sys.exit(1)
