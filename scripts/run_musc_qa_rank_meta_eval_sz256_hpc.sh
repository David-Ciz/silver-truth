...
CONFIG="experiments/variants/musc_qa_rank_meta_eval_sz256.yaml"
...
usage() {
    cat <<'EOF'
Usage:
  scripts/run_musc_qa_rank_meta_eval_sz256_hpc.sh [options]

This submits:
1. fold-1 MuSC sz256 QA ranking+metadata evaluation
2. fold-2 MuSC sz256 QA ranking+metadata evaluation
3. a dependent comparison job after both folds succeed

Options:
  --config PATH          Variant YAML to submit. Default: experiments/variants/musc_qa_rank_meta_eval_sz256.yaml
...
submit_fold_job() {
    local fold="$1"
    local job_name="musc_qa_rank_meta_${fold}"
    local variant_name
...
compare_cmd=(
    sbatch
    --parsable
    --job-name musc_qa_rank_meta_compare
