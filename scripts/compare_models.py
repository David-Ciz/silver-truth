...
def analyze_model(excel_file, model_name):
    ...
    for df in [train_df, val_df, test_df]:
        if 'Jaccard index' in df.columns:
            df.rename(columns={'Jaccard index': 'jaccard_score',
                               'Predicted Jaccard index': 'predicted_jaccard'}, inplace=True)
    ...
    for name, df in [('Train', train_df), ('Validation', val_df), ('Test', test_df)]:
        ...
        print(f'{name:12} R2={r2:7.4f}  MAE={mae:.4f}  RMSE={rmse:.4f}  Pearson={pearson_r:.4f}  Spearman={spearman_r:.4f}')
    ...
    print(f'Predictions > 1.0: {(all_df["predicted_jaccard"] > 1.0).sum()} ({(all_df["predicted_jaccard"] > 1.0).sum()/len(all_df)*100:.1f}%)')
    ...
    for label in labels:
        ...
        if len(subset) > 0:
            print(f'  Actual {label}: n={len(subset):4d}, predicted mean={subset["predicted_jaccard"].mean():.3f}, '
    ...
    print()
    print(f'{"Pred Thresh":<12} {"Kept":<8} {"% Data":<8} {"Mean Actual":<12} {"% Excellent":<12} {"% Good+":<10}')
    print('-'*70)
    ...
    for pt in pred_thresholds:
        ...
        pct_excellent = (kept['jaccard_score'] >= 0.9).sum() / n_kept * 100 if n_kept > 0 else 0
        pct_good_plus = (kept['jaccard_score'] >= 0.8).sum() / n_kept * 100 if n_kept > 0 else 0

        print(f'>= {pt:<9} {n_kept:<8} {pct_kept:<8.1f} {mean_actual:<12.4f} {pct_excellent:<12.1f} {pct_good_plus:<10.1f}')
    ...
    if len(kept) > 0:
        ...
        print(f'    - {(kept["jaccard_score"] >= 0.8).sum()/len(kept)*100:.1f}% are good or better')
    else:
...
print('#'*70 + '\n')
old_df = analyze_model('results_resnet50_fixed.xlsx', 'OLD MODEL (results_resnet50_fixed)')

print('\n' + '#'*70)
print('#' + ' '*18 + 'NEW MODEL (no sigmoid)' + ' '*26 + '#')
print('#'*70 + '\n')
new_df = analyze_model('results/results_resnet50_local.xlsx', 'NEW MODEL - No Sigmoid (results_resnet50_local)')

print('\n' + '#'*70)
print('#' + ' '*18 + 'NEW MODEL (with sigmoid)' + ' '*24 + '#')
print('#'*70 + '\n')
sigmoid_df = analyze_model('results/results_resnet50_sigmoid.xlsx', 'NEW MODEL - With Sigmoid (results_resnet50_sigmoid)')

print('\n' + '#'*70)
print('#' + ' '*14 + 'NEW MODEL (sigmoid + weighted sampling)' + ' '*13 + '#')
print('#'*70 + '\n')
weighted_df = analyze_model('results/results_resnet50_sigmoid_weighted.xlsx', 'NEW MODEL - Sigmoid + Weighted (results_resnet50_sigmoid_weighted)')
...
weighted_test = pd.read_excel('results/results_resnet50_sigmoid_weighted.xlsx', sheet_name='test')

# Standardize names
for df in [old_test, new_test, sigmoid_test, weighted_test]:
    df.rename(columns={'Jaccard index': 'jaccard_score', 'Predicted Jaccard index': 'predicted_jaccard'}, inplace=True)

print(f'{"Model":<30} {"MAE":<8} {"Spearman":<10} {"Pred Min":<10} {"Pred Max":<10} {"%Good+ @0.85":<12}')
print('-'*80)

for name, df in [('OLD', old_test), ('NEW (no sigmoid)', new_test), ('NEW (sigmoid)', sigmoid_test), ('NEW (sigmoid+weighted)', weighted_test)]:
    y_true = df['jaccard_score'].values
    y_pred = df['predicted_jaccard'].values
    ...
    pct_good = (filtered['jaccard_score'] >= 0.8).sum() / len(filtered) * 100 if len(filtered) > 0 else 0
    print(f'{name:<30} {mae:<8.4f} {spearman_r:<10.4f} {pred_min:<10.4f} {pred_max:<10.4f} {pct_good:<12.1f}')
