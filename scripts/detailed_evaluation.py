...
class DetailedCellEvaluator:
    ...
    def calculate_cell_jaccard_scores(
        self, gt_image: np.ndarray, pred_image: np.ndarray
    ) -> List[Dict]:
        """
    ...
    def extract_metadata_from_row(
        self, row: pd.Series, campaign_col: str = "campaign_number"
    ) -> Dict:
        """
    ...
    def evaluate_competitor_detailed(
        self, competitor: str, campaign_col: str = "campaign_number"
    ) -> pd.DataFrame:
        """
        ...
        logging.info(
            f"Evaluating {len(valid_rows)} images for competitor '{competitor}'"
        )
        ...
        logging.info(
            f"Completed evaluation: {processed_count} processed, {skipped_count} skipped"
        )
...
@click.option(
    "--competitor",
    help="Specific competitor to evaluate (if not specified, evaluates all)",
)
    ...
    try:
        ...
        for comp in competitors:
            ...
            if not detailed_results.empty:
                ...
                logging.info(
                    f"Completed {comp}: {len(detailed_results)} cells evaluated"
                )
            else:
        ...
        competitor_summary = (
            final_df.groupby("competitor")["jaccard_score"]
