...
This project is organized into four main workflows. You must run the **Preprocessing** workflow before any other.
...
**3.2. Detailed Cell-Level Evaluation (`scripts/detailed_evaluation.py`)**
Provides detailed Jaccard score evaluation for individual cells, storing results in parquet format for efficient analysis.

**Usage:**
```bash
python scripts/detailed_evaluation.py <path_to_dataset_dataframe> [OPTIONS]
```

**Examples:**
```bash
# Detailed evaluation for all competitors in a dataset
python scripts/detailed_evaluation.py "dataframes/BF-C2DL-MuSC_dataset_dataframe.parquet" --output "detailed_BF-C2DL-MuSC.parquet"

# Detailed evaluation for specific competitor
python scripts/detailed_evaluation.py "dataframes/BF-C2DL-MuSC_dataset_dataframe.parquet" --competitor "MU-Lux-CZ" --output "detailed_BF-C2DL-MuSC_MU-Lux-CZ.parquet"

# Run detailed evaluation for all datasets automatically
python scripts/run_all_detailed_evaluations.py
```
...
**3.3. Analyze Detailed Results (`scripts/analyze_detailed_results.py`)**
Analyzes detailed cell evaluation results and generates comprehensive reports and visualizations.

**Usage:**
```bash
python scripts/analyze_detailed_results.py <path_to_detailed_results_parquet> [OPTIONS]
```

**Examples:**
```bash
# Analyze detailed results with default settings
python scripts/analyze_detailed_results.py "detailed_BF-C2DL-MuSC.parquet"

# Analyze with custom output directory and save summary
python scripts/analyze_detailed_results.py "detailed_BF-C2DL-MuSC.parquet" --output-dir "analysis_BF-C2DL-MuSC" --save-summary
```
...
```bash
# Convert single file
python scripts/convert_qa_to_parquet.py convert-csv "qa_jaccard_results_BF-C2DL-MuSC.csv" "qa_BF-C2DL-MuSC_dataset.parquet" --output "qa_BF-C2DL-MuSC_detailed_cells.parquet"

# Batch convert all QA results in current directory
python scripts/convert_qa_to_parquet.py batch-convert

# Use analyze_detailed_results.py on converted QA parquet files
python scripts/analyze_detailed_results.py "qa_BF-C2DL-MuSC_detailed_cells.parquet"
```
...
#### Visualization
```bash
python src/vizualizations/visualize_tif.py "fused_results/BF-C2DL-MuSC_01_fused_0001.tif"
python src/vizualizations/show_objects.py "fused_results/BF-C2DL-MuSC_01_fused_0001.tif"
```
